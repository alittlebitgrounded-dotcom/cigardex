...
type Cigar = {
  ...
  country_of_origin: string | null; created_at: string
...
export default function BrowseNewest() {
  ...
  async function fetchCigars() {
    ...
    const { data } = await supabase
      .from('cigars')
      .select('id, name, line, vitola, strength, msrp, status, country_of_origin, created_at, is_discontinued, brand_account_id, brand_accounts!left(id, name)')
      .eq('status', 'live')
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#faf8f5', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ background: '#f0e8dc', padding: '10px 32px', fontSize: 13, color: '#8b5e2a' }}>
        ...
        <span style={{ color: '#1a0a00', fontWeight: 500 }}>Newest Additions</span>
      </div>
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '32px 24px 48px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 700, color: '#1a0a00', margin: '0 0 6px' }}>🆕 Newest Additions</h1>
            <p style={{ fontSize: 14, color: '#8b5e2a', margin: 0 }}>
              {loading ? 'Loading...' : `${filtered.length} cigars, sorted by date added`}
            </p>
          </div>
              ...
              {paged.map(cigar => {
                ...
                return (
                  <a key={cigar.id} href={`/cigar/${cigar.id}`} style={{ textDecoration: 'none' }}>
                    <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e8ddd0', padding: 20, height: '100%', boxSizing: 'border-box', boxShadow: '0 2px 6px rgba(0,0,0,0.06)', transition: 'box-shadow 0.15s, transform 0.15s', cursor: 'pointer', opacity: disc ? 0.85 : 1 }}
                    ...
                    >
                      <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                        <div style={{ display: 'inline-block', background: '#1a0a00', color: '#c4a96a', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 4, letterSpacing: '0.06em' }}>NEW</div>
                        {disc && <span style={{ background: '#F7C1C1', color: '#791F1F', fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 3 }}>Discontinued</span>}
                      </div>
