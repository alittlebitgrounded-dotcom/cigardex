...
function ratingColor(avg: number): string {
  if (avg >= 9) return '#2e7d32'; if (avg >= 8) return '#558b2f'
  if (avg >= 7) return '#f57f17'; return '#8b5e2a'
}
...
export default function BrowseHighestRated() {
  ...
  async function fetchCigars() {
    setLoading(true)
    const { data: reviewData } = await supabase.from('reviews').select('cigar_id, rating').not('rating', 'is', null)
    ...
    reviewData.forEach(r => {
      ...
      sums[r.cigar_id].total += r.rating; sums[r.cigar_id].count++
    })

    const ranked = Object.entries(sums)
      .filter(([, v]) => v.count >= 1)
      .map(([id, v]) => ({ id, avg: v.total / v.count, count: v.count }))
      .sort((a, b) => b.avg - a.avg).slice(0, 300)
    ...
    if (data) {
      const withRatings = ranked
        .map(r => data.find(c => c.id === r.id) ? { ...data.find(c => c.id === r.id), avg_rating: r.avg, review_count: r.count } : null)
        .filter(Boolean) as unknown as Cigar[]
      setCigars(withRatings)
    }
    setLoading(false)
  }

  const filtered = (filterCigars(cigars) as Cigar[])
    .filter(c => c.review_count >= minReviews)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#faf8f5', fontFamily: 'Georgia, serif' }}>
      ...
      <div style={{ background: '#f0e8dc', padding: '10px 32px', fontSize: 13, color: '#8b5e2a' }}>
        ...
        <span style={{ color: '#1a0a00', fontWeight: 500 }}>Highest Rated</span>
      </div>
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '32px 24px 48px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 700, color: '#1a0a00', margin: '0 0 6px' }}>⭐ Highest Rated</h1>
            <p style={{ fontSize: 14, color: '#8b5e2a', margin: 0 }}>
              {loading ? 'Loading...' : `${filtered.length} cigars ranked by community rating`}
            </p>
          </div>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', border: '1px solid #e8ddd0', borderRadius: 8, padding: '10px 16px' }}>
              <span style={{ fontSize: 13, color: '#5a3a1a', fontWeight: 500 }}>Min reviews:</span>
              {[1, 2, 5, 10].map(n => (
                <button key={n} onClick={() => { setMinReviews(n); setPage(1) }}
                  style={{ padding: '4px 12px', borderRadius: 4, border: 'none', fontSize: 13, fontWeight: minReviews === n ? 700 : 400, background: minReviews === n ? '#1a0a00' : '#f5f0e8', color: minReviews === n ? '#f5e6c8' : '#5a3a1a', cursor: 'pointer' }}>
                  {n}+
                </button>
              ))}
            </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 0', color: '#aaa' }}>
            <p style={{ fontSize: 16 }}>No cigars match your filters</p>
            <button onClick={() => { setMinReviews(1); setHideDiscontinued(false) }} style={{ marginTop: 12, padding: '8px 18px', borderRadius: 6, background: '#f5f0e8', border: '1px solid #d4b896', color: '#5a3a1a', fontSize: 13, cursor: 'pointer' }}>Reset filters</button>
          </div>
        ) : (
          <>
              ...
              {paged.map((cigar, i) => {
                ...
                return (
                  <a key={cigar.id} href={`/cigar/${cigar.id}`} style={{ textDecoration: 'none' }}>
                    ...
                    >
                      {globalRank <= 3 && (
                        <div style={{ position: 'absolute', top: 12, right: 12, width: 28, height: 28, borderRadius: '50%', background: globalRank === 1 ? '#c4a96a' : globalRank === 2 ? '#aaa' : '#c47a4a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#fff' }}>
                          {globalRank}
                        </div>
                      )}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid #f0e8dc', paddingTop: 10 }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: 20, fontWeight: 700, color: ratingColor(cigar.avg_rating), lineHeight: 1 }}>{cigar.avg_rating.toFixed(1)}</div>
                          <div style={{ fontSize: 10, color: '#aaa' }}>{cigar.review_count} review{cigar.review_count !== 1 ? 's' : ''}</div>
                        </div>
                      </div>
                    </div>
                  </a>
