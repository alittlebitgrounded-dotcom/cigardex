...
async function rejectTimelineEntry(id: string) {
  ...
  // ← PASTE THE 4 NEW FUNCTIONS RIGHT HERE ↓
  ...
  function buildTimelinePayload() {
    ...
    if (month && day) {
      ...
      event_date = `${year}-${month}-${paddedDay}`
      date_precision = 'day'
    }
  ...
  async function saveTimelineEdit(id: string) {
    try {
      ...
      if (error) {
        setMsg(`Timeline save failed: ${error.message}`)
        return
      }
  ...
  async function saveAndApproveTimelineEdit(id: string) {
    try {
      ...
      const { error } = await supabase
        .from('cigar_timeline')
        .update({ ...payload, status: 'live' })
        .eq('id', id)

      if (error) {
        setMsg(`Timeline save/approve failed: ${error.message}`)
        return
      }
  ...
  async function startTimelineMerge(entry: any) {
    setTimelineMergeId(entry.id)
    setTimelineMergeTargetId(null)
    setTimelineMergeFields({})
    setTimelineSaving(false)

    const { data, error } = await supabase
      .from('cigar_timeline')
      .select('id, cigar_id, event_type, event_date, date_precision, title, body, source, status, created_at')
      .eq('cigar_id', entry.cigar_id)
      .eq('status', 'live')
      .neq('id', entry.id)
      .order('event_date', { ascending: true })

    if (error) {
      setTimelineMergeTargets([])
      setMsg(`Could not load merge targets: ${error.message}`)
      return
    }
  ...
  async function executeMergeTimeline() {
    if (!timelineMergeId || !timelineMergeTargetId) {
      setMsg('Pick a target entry first')
      return
    }
    ...
    if (!sourceEntry || !targetEntry) {
      setMsg('Merge source or target could not be found')
      return
    }

    const selectedFields = Object.entries(timelineMergeFields)
      .filter(([, checked]) => checked)
      .map(([key]) => key)

    if (selectedFields.length === 0) {
      setMsg('Choose at least one field to copy')
      return
    }
    ...
    try {
      ...
      for (const field of selectedFields) {
        if (field === 'event_date') {
          updatePayload.event_date = sourceEntry.event_date
          updatePayload.date_precision = sourceEntry.date_precision
        } else {
          updatePayload[field] = sourceEntry[field]
        }
      }
      ...
      const { error: updateError } = await supabase
        ...
        .eq('id', timelineMergeTargetId)

      if (updateError) {
        setMsg(`Merge update failed: ${updateError.message}`)
        return
      }
      ...
      const { error: deleteError } = await supabase
        .from('cigar_timeline')
        .delete()
        .eq('id', timelineMergeId)

      if (deleteError) {
        setMsg(`Merged target updated, but pending entry was not removed: ${deleteError.message}`)
        return
      }

      setTimelineEntries(prev => prev.filter(e => e.id !== timelineMergeId))
      setTimelineMergeId(null)
      setTimelineMergeTargets([])
      setTimelineMergeTargetId(null)
      setTimelineMergeFields({})
      setMsg('Timeline entry merged into existing live entry')

      await logAction(
        'merge_timeline',
        'cigar_timeline',
        timelineMergeId,
        `Merged pending entry into ${timelineMergeTargetId}; fields copied: ${selectedFields.join(', ')}`
      )
    } catch (err: any) {
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                {[
                  ...
                  return (
                    <button key={key}
                      onClick={() => setCigarIncompleteFilter(prev => prev.includes(key) ? prev.filter(k => k !== key) : [...prev, key])}
                  ...
                  <tbody>
                    {filteredCigars.map((c, i) => (
                      <tr key={c.id} style={{ borderBottom: '1px solid #f0e8dc', background: i % 2 === 0 ? '#fff' : '#faf8f5' }}>
                        <td style={{ padding: '10px 14px', fontSize: 13, color: '#c4a96a', fontWeight: 500 }}>{c.brand_accounts?.name}</td>
                  ...
                  <tbody>
                    {filteredBrands.map((b, i) => (
                      <tr key={b.id} style={{ borderBottom: '1px solid #f0e8dc', background: i % 2 === 0 ? '#fff' : '#faf8f5' }}>
                        <td style={{ padding: '10px 14px', fontSize: 14, fontWeight: 600, color: '#1a0a00' }}>{b.name}</td>
          ...
          return (
            <div key={entry.id} style={{ background: '#fff', borderRadius: 10, border: '1px solid #e8ddd0', padding: 20 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <div>
                  <p style={{ fontSize: 13, color: '#c4a96a', fontWeight: 600, margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  ...
                  <p style={{ fontSize: 12, color: '#8b5e2a', margin: 0 }}>
                    {entry.event_type.replace('_', ' ')} · {entry.event_date ? entry.event_date.split('T')[0] : 'No date'} · by {entry._username} · {new Date(entry.created_at).toLocaleDateString()}
                  </p>
                </div>
                {!isEditing && !isMerging && (
                  <div style={{ display: 'flex', gap: 8, flexShrink: 0, marginLeft: 16, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                    ...
                    <button onClick={() => {
                      ...
                      setTimelineMergeId(null)
                      const dateParts = entry.event_date ? entry.event_date.split('T')[0].split('-') : ['', '', '']
                      setTimelineEditForm({
                        event_type: entry.event_type,
                        year: dateParts[0] || '',
                        month: entry.date_precision !== 'year' ? (dateParts[1] || '') : '',
                        day: entry.date_precision === 'day' ? (dateParts[2] || '') : '',
                        title: entry.title || '',
                        body: entry.body || '',
                        source: entry.source || '',
                      })
                    }} style={btnWarning}>✏️ Edit</button>
              ...
              {entry.body && !isEditing && (
                <p style={{ fontSize: 13, color: '#444', lineHeight: 1.6, margin: '0 0 8px', background: '#faf8f5', padding: '10px 14px', borderRadius: 6 }}>{entry.body}</p>
              )}
              {entry.source && !isEditing && (
                <p style={{ fontSize: 12, color: '#8b5e2a', margin: 0 }}>
                  Source: <a href={entry.source.startsWith('http') ? entry.source : undefined} target="_blank" rel="noopener noreferrer" style={{ color: '#c4a96a' }}>{entry.source}</a>
                </p>
              )}
                ...
                <div style={{ borderTop: '1px solid #e8ddd0', marginTop: 12, paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <select value={timelineEditForm.event_type} onChange={e => setTimelineEditForm(p => ({ ...p, event_type: e.target.value }))} style={inputStyle}>
                      {['release', 'discontinued', 'blend_change', 'name_change', 'size_change', 'award', 'price_change', 'note'].map(t => (
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr', gap: 10 }}>
                    <div>
                      <label style={{ fontSize: 12, color: '#8b5e2a', display: 'block', marginBottom: 4 }}>Year</label>
                      <input type="number" value={timelineEditForm.year} onChange={e => setTimelineEditForm(p => ({ ...p, year: e.target.value }))} placeholder="e.g. 2015" style={inputStyle} />
                    </div>
                    <div>
                    ...
                    <div>
                      <label style={{ fontSize: 12, color: '#8b5e2a', display: 'block', marginBottom: 4 }}>Day</label>
                      <input type="number" min="1" max="31" value={timelineEditForm.day} onChange={e => setTimelineEditForm(p => ({ ...p, day: e.target.value }))} placeholder="—" style={inputStyle} />
                    </div>
                  </div>

                  <div>
                    <label style={{ fontSize: 12, color: '#8b5e2a', display: 'block', marginBottom: 4 }}>Title</label>
                    <input value={timelineEditForm.title} onChange={e => setTimelineEditForm(p => ({ ...p, title: e.target.value }))} style={inputStyle} />
                  </div>

                  <div>
                    <label style={{ fontSize: 12, color: '#8b5e2a', display: 'block', marginBottom: 4 }}>Details (optional)</label>
                    <textarea value={timelineEditForm.body} onChange={e => setTimelineEditForm(p => ({ ...p, body: e.target.value }))} rows={3} style={{ ...inputStyle, resize: 'vertical', fontFamily: 'system-ui', lineHeight: 1.6 }} />
                  </div>

                  <div>
                    <label style={{ fontSize: 12, color: '#8b5e2a', display: 'block', marginBottom: 4 }}>Source (optional)</label>
                    <input value={timelineEditForm.source} onChange={e => setTimelineEditForm(p => ({ ...p, source: e.target.value }))} style={inputStyle} />
                  </div>

                  <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
                    <button onClick={() => saveAndApproveTimelineEdit(entry.id)} disabled={timelineSaving} style={{ ...btnSuccess, padding: '8px 18px', fontSize: 13 }}>
                      {timelineSaving ? 'Saving...' : '✓ Save & Approve'}
                    </button>
                    <button onClick={() => saveTimelineEdit(entry.id)} disabled={timelineSaving} style={{ ...btnWarning, padding: '8px 18px', fontSize: 13 }}>
                      Save edits only
                    </button>
                    <button onClick={() => setTimelineEditId(null)} style={{ background: 'none', border: '1px solid #ddd', borderRadius: 6, padding: '8px 16px', fontSize: 13, color: '#888', cursor: 'pointer' }}>
                      Cancel
                    </button>
                  </div>
                </div>
              ...
              {isMerging && (
                <div style={{ borderTop: '1px solid #e8ddd0', marginTop: 12, paddingTop: 16 }}>
                  <p style={{ fontSize: 12, fontWeight: 600, color: '#6a1b9a', margin: '0 0 12px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                    🔀 Merge into existing entry — pick which fields to copy from this submission
                  </p>
                  ...
                  {timelineMergeTargets.length === 0 ? (
                    ...
                    <>
                      ...
                      {timelineMergeTargetId && (() => {
                        ...
                        return (
                          <div style={{ marginBottom: 14 }}>
                            <label style={{ fontSize: 12, color: '#8b5e2a', display: 'block', marginBottom: 8 }}>Step 2 — Choose which fields to copy from the pending submission:</label>
                            <div style={{ background: '#faf8f5', borderRadius: 8, border: '1px solid #e8ddd0', overflow: 'hidden' }}>
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', background: '#f5f0e8', padding: '8px 14px', fontSize: 11, fontWeight: 600, color: '#5a3a1a', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                                <span>Field</span><span>Existing</span><span>Pending (new)</span><span>Copy?</span>
                              </div>
                              {fields.map(f => (
                                <div key={f.key} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', padding: '10px 14px', borderTop: '1px solid #e8ddd0', alignItems: 'center', background: timelineMergeFields[f.key] ? '#f3e5f5' : '#fff' }}>
                                  ...
                                  <input type="checkbox" checked={!!timelineMergeFields[f.key]}
                                    onChange={e => setTimelineMergeFields(prev => ({ ...prev, [f.key]: e.target.checked }))}
                  ...
                  <div style={{ display: 'flex', gap: 10 }}>
                    {timelineMergeTargetId && Object.values(timelineMergeFields).some(Boolean) && (
                      <button onClick={executeMergeTimeline} disabled={timelineSaving}
                        style={{ background: '#6a1b9a', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                        {timelineSaving ? 'Merging...' : '🔀 Execute Merge'}
                      </button>
                    )}
                    <button onClick={() => { setTimelineMergeId(null); setTimelineMergeTargets([]); setTimelineMergeTargetId(null); setTimelineMergeFields({}) }}
                      style={{ background: 'none', border: '1px solid #ddd', borderRadius: 6, padding: '8px 16px', fontSize: 13, color: '#888', cursor: 'pointer' }}>
                      Cancel
                    </button>
                  </div>
                </div>
                    ...
                    <div key={entry.id} style={{ background: '#fff', borderRadius: 10, border: '1px solid #e8ddd0', padding: 20 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                        <div>
                          ...
                          <p style={{ fontSize: 15, fontWeight: 700, color: '#1a0a00', margin: '0 0 4px' }}>{entry.title}</p>
                          <p style={{ fontSize: 12, color: '#8b5e2a', margin: 0 }}>
                            {entry.event_type.replace('_', ' ')} · {entry.event_date ? entry.event_date.split('T')[0] : 'No date'} · by {entry._username} · submitted {new Date(entry.created_at).toLocaleDateString()}
                          </p>
                        </div>
