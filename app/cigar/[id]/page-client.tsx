...
import {
  getReviewerPublicationProfilesForUsers,
  resolveIndustryType,
...
export default function CigarDetailPage() {
  ...
  async function loadUserRole(userId: string) {
    ...
    const membership = await resolveIndustryType(userId, data?.role ?? null)
    setCurrentUserRole(membership.industryType)
  }
  ...
  async function fetchAll() {
    ...
    if (reviewRes.data) {
      ...
      const reviewIds = rawReviews.map(r => r.id)
        ...
        if (userIds.length > 0) {
          const [userRowsResult, reviewerPublications] = await Promise.all([
            supabase.from('users').select('id, username, publication_name, role').in('id', userIds),
            getReviewerPublicationProfilesForUsers(userIds),
          ])
          const userRows = userRowsResult.data
          if (userRows) userRows.forEach((u: any) => {
            userDataMap[u.id] = {
              username: u.username,
              publication_name: reviewerPublications[u.id]?.publicationName || u.publication_name || null,
              role: reviewerPublications[u.id] ? 'reviewer' : u.role,
            }
          })
        }
