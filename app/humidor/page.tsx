...
export default function HumidorPage() {
  ...
  async function checkAuth() {
    ...
    fetchHumidor(session.user.id)
  }

  async function fetchHumidor(uid: string) {
    setLoading(true)
    const { data } = await supabase
      .from('humidor_items')
      .select('id, quantity, purchase_price, purchase_date, notes, added_at, cigars(id, name, vitola, strength, brand_accounts(id, name))')
      .eq('user_id', uid)
    ...
    if (data) {
      ...
      if (cigarIds.length > 0) {
        const { data: reviews } = await supabase
          .from('reviews')
          .select('cigar_id, rating')
          .eq('user_id', uid)
          .in('cigar_id', cigarIds)
          .not('rating', 'is', null)

        reviews?.forEach(r => {
          if (r.cigar_id && r.rating) ratingsMap[r.cigar_id] = r.rating
        })
      }
  ...
  async function moveToWishlist(item: HumidorItem) {
    if (!userId || !item.cigars) return
    await supabase.from('humidor_items').delete().eq('id', item.id)
    await supabase.from('wishlist_items').upsert({ user_id: userId, cigar_id: item.cigars.id }, { onConflict: 'user_id,cigar_id' })
    setItems(prev => prev.filter(i => i.id !== item.id))
    setActionMsg(`Moved "${item.cigars?.name}" to wishlist`)
  ...
  async function removeFromHumidor(item: HumidorItem) {
    await supabase.from('humidor_items').delete().eq('id', item.id)
    setItems(prev => prev.filter(i => i.id !== item.id))
    setActionMsg(`Removed "${item.cigars?.name}" from humidor`)
    setTimeout(() => setActionMsg(''), 3000)
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#faf8f5', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ background: '#f0e8dc', padding: '10px 32px', fontSize: 13, color: '#8b5e2a' }}>
        ...
        <span style={{ color: '#1a0a00', fontWeight: 500 }}>My Humidor</span>
      </div>
      ...
      <div style={{ background: 'linear-gradient(135deg, #2c1206 0%, #1a0a00 100%)', padding: '32px 32px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 24 }}>
            <div>
              <h1 style={{ color: '#f5e6c8', fontSize: 28, fontWeight: 700, margin: '0 0 8px' }}>My Humidor</h1>
              <div style={{ display: 'flex', gap: 20 }}>
                <span style={{ color: '#c4a96a', fontSize: 14 }}>🗃 {items.length} cigar{items.length !== 1 ? 's' : ''}</span>
                <span style={{ color: '#c4a96a', fontSize: 14 }}>🍂 {totalCigars} total sticks</span>
                {totalValue > 0 && <span style={{ color: '#c4a96a', fontSize: 14 }}>💰 ${totalValue.toFixed(2)} invested</span>}
              </div>
            </div>
            {/* View toggle */}
          ...
          {/* Nav tabs — matches brand/profile pages */}
          <div style={{ display: 'flex', borderBottom: '1px solid rgba(196,169,106,0.2)' }}>
            ...
            <a href="/Wishlist" style={{ padding: '12px 20px', background: 'none', border: 'none', borderBottom: '3px solid transparent', color: '#8b6a4a', fontSize: 14, fontWeight: 400, cursor: 'pointer', marginBottom: -1, textDecoration: 'none' }}>
              Wishlist
            </a>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 24px 48px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 20, alignItems: 'center' }}>
            ...
            {(['added', 'name', 'brand', 'price'] as const).map(s => (
              <button key={s} onClick={() => setSortBy(s)} style={{
                ...
                {s === 'added' ? 'Date Added' : s === 'name' ? 'Name' : s === 'brand' ? 'Brand' : 'Price'}
              </button>
            ))}
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '80px 0', color: '#8b5e2a' }}>Loading your humidor...</div>
        ) : items.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>🗃</div>
            <p style={{ fontSize: 18, color: '#8b5e2a', marginBottom: 8 }}>Your humidor is empty</p>
            <p style={{ fontSize: 14, color: '#aaa', marginBottom: 24 }}>Add cigars from any cigar detail page</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
            {sorted.map(item => <HumidorCard key={item.id} item={item} onMoveToWishlist={moveToWishlist} onRemove={removeFromHumidor} />)}
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {sorted.map(item => <HumidorListItem key={item.id} item={item} onMoveToWishlist={moveToWishlist} onRemove={removeFromHumidor} />)}
          </div>
        )}
      </div>
...
function HumidorCard({ item, onMoveToWishlist, onRemove }: {
  item: HumidorItem
  onMoveToWishlist: (item: HumidorItem) => void
  onRemove: (item: HumidorItem) => void
}) {
    ...
    <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e8ddd0', padding: 20, boxShadow: '0 2px 6px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div>
        ...
        <a href={`/cigar/${item.cigars?.id}`} style={{ textDecoration: 'none' }}>
          <h3 style={{ color: '#1a0a00', fontSize: 16, fontWeight: 700, margin: '0 0 6px', lineHeight: 1.3 }}>{item.cigars?.name}</h3>
        </a>
      ...
      {/* Details */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, borderTop: '1px solid #f0e8dc', paddingTop: 12 }}>
        <div>
          <p style={{ fontSize: 11, color: '#aaa', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Quantity</p>
          <p style={{ fontSize: 15, fontWeight: 700, color: '#1a0a00', margin: 0 }}>{item.quantity}</p>
        </div>
        {item.purchase_price && (
          <div>
            <p style={{ fontSize: 11, color: '#aaa', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Paid each</p>
            <p style={{ fontSize: 15, fontWeight: 700, color: '#1a0a00', margin: 0 }}>${item.purchase_price.toFixed(2)}</p>
          </div>
        )}
        {item.purchase_date && (
          <div>
            <p style={{ fontSize: 11, color: '#aaa', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Purchased</p>
            <p style={{ fontSize: 13, color: '#5a3a1a', margin: 0 }}>{new Date(item.purchase_date).toLocaleDateString()}</p>
          </div>
        )}
        <div>
          <p style={{ fontSize: 11, color: '#aaa', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Added</p>
          <p style={{ fontSize: 13, color: '#5a3a1a', margin: 0 }}>{new Date(item.added_at).toLocaleDateString()}</p>
        </div>
      </div>

      {item.notes && (
        <p style={{ fontSize: 13, color: '#666', margin: 0, fontStyle: 'italic', borderTop: '1px solid #f0e8dc', paddingTop: 10 }}>{item.notes}</p>
      )}

      {/* Actions */}
      <div style={{ display: 'flex', gap: 8, borderTop: '1px solid #f0e8dc', paddingTop: 12 }}>
        <a href={`/cigar/${item.cigars?.id}`} style={{ flex: 1, padding: '8px 0', textAlign: 'center', background: '#f5f0e8', color: '#5a3a1a', borderRadius: 6, fontSize: 13, fontWeight: 500, textDecoration: 'none' }}>
          {item.userRating ? 'Edit Review' : 'Review'}
        </a>
        <button onClick={() => onMoveToWishlist(item)} style={{ flex: 1, padding: '8px 0', background: '#fff3e0', color: '#e65100', border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
          → Wishlist
        </button>
        {!showConfirm ? (
          <button onClick={() => setShowConfirm(true)} style={{ padding: '8px 12px', background: '#fbe9e7', color: '#b71c1c', border: 'none', borderRadius: 6, fontSize: 13, cursor: 'pointer' }}>
            ✕
          </button>
        ) : (
          <button onClick={() => onRemove(item)} style={{ padding: '8px 12px', background: '#b71c1c', color: '#fff', border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            Confirm
          </button>
        )}
      </div>
...
function HumidorListItem({ item, onMoveToWishlist, onRemove }: {
  item: HumidorItem
  onMoveToWishlist: (item: HumidorItem) => void
  onRemove: (item: HumidorItem) => void
}) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e8ddd0', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 20 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 8, fontSize: 12, color: '#8b5e2a', flexWrap: 'wrap' }}>
          {item.cigars?.vitola && <span>{item.cigars.vitola}</span>}
          {item.purchase_date && <span>· Purchased {new Date(item.purchase_date).toLocaleDateString()}</span>}
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexShrink: 0 }}>
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 11, color: '#aaa', margin: '0 0 2px' }}>QTY</p>
          <p style={{ fontSize: 18, fontWeight: 700, color: '#1a0a00', margin: 0 }}>{item.quantity}</p>
        </div>
        {item.purchase_price && (
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: 11, color: '#aaa', margin: '0 0 2px' }}>PAID</p>
            <p style={{ fontSize: 15, fontWeight: 600, color: '#1a0a00', margin: 0 }}>${item.purchase_price.toFixed(2)}</p>
          </div>
        )}
        {item.userRating && (
          <div style={{ background: '#1a0a00', color: '#c4a96a', padding: '4px 10px', borderRadius: 6, fontSize: 14, fontWeight: 700 }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
        <a href={`/cigar/${item.cigars?.id}`} style={{ padding: '7px 14px', background: '#f5f0e8', color: '#5a3a1a', borderRadius: 6, fontSize: 13, fontWeight: 500, textDecoration: 'none' }}>
          {item.userRating ? 'Edit Review' : 'Review'}
        </a>
        <button onClick={() => onMoveToWishlist(item)} style={{ padding: '7px 14px', background: '#fff3e0', color: '#e65100', border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
          → Wishlist
        </button>
        {!showConfirm ? (
