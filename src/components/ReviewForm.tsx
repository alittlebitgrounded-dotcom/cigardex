...
function DescriptiveSlider({
  label, value, onChange, getLabel, getDescription, getColor, leftNote, rightNote,
}: {
  label: string
  value: number | null
  onChange: (v: number) => void
  getLabel: (v: number) => string
  getDescription: (v: number) => string
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 10, padding: 20, border: '1px solid #e8ddd0', marginBottom: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: active ? 6 : 12 }}>
        ...
        {active ? (
          <span style={{ fontSize: 15, fontWeight: 700, color: getColor(value!) }}>
            {getLabel(value!)}
          </span>
        ) : (
      ...
      {active && (
        <p style={{ fontSize: 12, color: '#8b5e2a', margin: '0 0 12px', fontStyle: 'italic' }}>
      ...
      {!active && (
        <p style={{ fontSize: 11, color: '#bbb', margin: '6px 0 0', textAlign: 'center' }}>
          ...
          return (
            <button
              ...
              style={{
                ...
                background: sel ? '#1a0a00' : '#f5f0e8',
                color: sel ? '#f5e6c8' : '#5a3a1a',
                border: sel ? '1px solid #1a0a00' : '1px solid #d4b896',
                cursor: 'pointer',
              }}
            >
              {opt}
...
export default function ReviewForm({
  ...
  const [smokedAt, setSmokedAt] = useState(
    existingReview?.smoked_at ? existingReview.smoked_at.split('T')[0] : new Date().toISOString().split('T')[0]
  )
  ...
  useEffect(() => {
    ...
    void fetchPairingCategories()
  }, [])

  useEffect(() => {
    if (pairingCategoryId) {
      void fetchPairingSubcategories(pairingCategoryId)
    } else {
      setPairingSubcategories([])
      setPairingSubcategoryId('')
    }
  }, [pairingCategoryId])
  ...
  useEffect(() => {
    if (!existingReview) {
      setSelectedChars([])
      setOriginalSelectedChars([])
      setPairingCategoryId('')
      setPairingSubcategoryId('')
      setPairingSpecific('')
      return
    }
    ...
    async function loadExistingExtras() {
      const { data: characteristicRows, error: characteristicError } = await supabase
      ...
      const loadedCharIds = (characteristicRows ?? [])
        .map(row => row.characteristic_id as string)
        .filter(Boolean)

      setSelectedChars(loadedCharIds)
      setOriginalSelectedChars(loadedCharIds)

      const { data: pairingRow, error: pairingError } = await supabase
        .from('review_pairings')
        .select('category_id, subcategory_id, specific_id, free_text')
        .eq('review_id', existingReview.id)
        .maybeSingle()

      if (pairingError) throw pairingError

      setPairingCategoryId(pairingRow?.category_id ?? '')
      setPairingSubcategoryId(pairingRow?.subcategory_id ?? '')
      setPairingSpecific(pairingRow?.free_text ?? '')
    }

    loadExistingExtras().catch(err => {
      setError(err instanceof Error ? err.message : 'Could not load existing review details')
    })
  }, [existingReview])
  ...
  async function fetchCharacteristics() {
    const { data, error } = await supabase
      ...
      .select('id, canonical_name, category, vote_count, status')
      .eq('status', 'active')
  ...
  async function handleSubmit() {
    if (rating === null) {
      setError('Please set an overall rating before submitting.')
      return
    }
    ...
    try {
      ...
      const reviewData = {
        ...
        strength_impression: strengthImpression || null,
        body: body || null,
        finish: finish.length ? finish.join(',') : null,
        occasion: occasion || null,
        where_smoked: whereSmoked || null,
      ...
      if (existingReview) {
        ...
        const { data, error: insertReviewError } = await supabase
          .from('reviews')
          .insert(reviewData)
          .select('id')
          .single()
      ...
      if (existingReview) {
        const { error: deleteReviewCharsError } = await supabase
          .from('review_characteristics')
          .delete()
          .eq('review_id', reviewId)

        if (deleteReviewCharsError) throw deleteReviewCharsError
      }

      if (selectedChars.length > 0) {
        const { error: insertReviewCharsError } = await supabase
          .from('review_characteristics')
          .insert(selectedChars.map(cid => ({ review_id: reviewId, characteristic_id: cid })))

        if (insertReviewCharsError) throw insertReviewCharsError
      }

      for (const cid of addedCharIds) {
        await upsertCigarCharacteristicVote(cid)
      }

      for (const cid of removedCharIds) {
        await removeCigarCharacteristicVote(cid)
      }

      if (!existingReview) {
        for (const cid of selectedChars.filter(id => !addedCharIds.includes(id))) {
          await upsertCigarCharacteristicVote(cid)
        }
      }

      if (freeTextChar.trim()) {
        const { data: newChar, error: createCharError } = await supabase
          .from('characteristics')
          .insert({
            raw_name: freeTextChar.trim(),
            canonical_name: freeTextChar.trim(),
            category: 'Flavor',
            status: 'unverified',
            vote_count: 1,
          })
        ...
        if (createCharError) throw createCharError

        const { error: addNewCharToReviewError } = await supabase
          .from('review_characteristics')
          .insert({
            review_id: reviewId,
            characteristic_id: newChar.id,
          })

        if (addNewCharToReviewError) throw addNewCharToReviewError
        await upsertCigarCharacteristicVote(newChar.id)
      }
      ...
      if (pairingCategoryId) {
        const { error: insertPairingError } = await supabase
          .from('review_pairings')
          .insert({
            review_id: reviewId,
            category_id: pairingCategoryId,
            subcategory_id: pairingSubcategoryId || null,
            specific_id: null,
            free_text: pairingSpecific.trim() || null,
          })

        if (insertPairingError) throw insertPairingError
      }
  ...
  const tabStyle = (active: boolean) => ({
    ...
    color: active ? '#1a0a00' : '#8b5e2a',
    background: 'none',
    border: 'none',
    borderBottom: active ? '2px solid #1a0a00' : '2px solid transparent',
    cursor: 'pointer',
  })
  ...
  const chipStyle = (selected: boolean) => ({
    ...
    background: selected ? '#1a0a00' : '#f5f0e8',
    color: selected ? '#f5e6c8' : '#5a3a1a',
    border: selected ? '1px solid #1a0a00' : '1px solid #d4b896',
    cursor: 'pointer',
  })
  ...
  return (
    <div style={{ background: '#faf8f5', border: '1px solid #e8ddd0', borderRadius: 12, padding: 28, marginTop: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        <button type="button" onClick={onCancel} style={{ background: 'none', border: 'none', color: '#aaa', fontSize: 20, cursor: 'pointer' }}>
          ✕
        </button>
      </div>
      ...
      {section === 'scores' && (
        <div>
          <div style={{ background: '#1a0a00', borderRadius: 12, padding: 24, marginBottom: 20, textAlign: 'center' }}>
            <p style={{ color: '#c4a96a', fontSize: 13, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 8px' }}>
              Your Overall Score
            </p>
            {rating !== null ? (
            ...
            <p style={{ color: '#888', fontSize: 12, margin: '6px 0 16px', fontStyle: 'italic' }}>
              {rating === null
                ...
                : rating >= 9
                  ? 'Exceptional — a benchmark cigar'
                  : rating >= 7
                    ? 'Very good — would seek it out again'
                    : rating >= 5
                      ? 'Decent — has its moments'
            ...
            <input
              ...
              onMouseDown={() => { if (rating === null) setRating(5) }}
              onTouchStart={() => { if (rating === null) setRating(5) }}
          ...
          <p style={{ fontSize: 13, color: '#8b5e2a', margin: '0 0 16px', fontStyle: 'italic' }}>
            The scores below don&apos;t affect your overall rating — they help the community understand the cigar better.
          </p>
          ...
          <DescriptiveSlider
            ...
            getColor={v => v <= 4 ? '#b71c1c' : v <= 6 ? '#e65100' : '#2e7d32'}
            leftNote="Not worth it"
            rightNote="Exceptional value"
          />
        </div>
      )}
        ...
        <div>
          <div style={{ background: '#fff', borderRadius: 10, padding: 20, marginBottom: 14, border: '1px solid #e8ddd0' }}>
            ...
            <textarea
              ...
              rows={4}
              style={{ width: '100%', padding: '12px 14px', fontSize: 14, border: '1px solid #d4b896', borderRadius: 8, outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'system-ui, sans-serif', lineHeight: 1.6 }}
            />
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: 10, padding: 20, marginBottom: 14, border: '1px solid #e8ddd0' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
              {topChars.map(c => (
                <button key={c.id} type="button" onClick={() => toggleChar(c.id)} style={chipStyle(selectedChars.includes(c.id))}>
                  {c.canonical_name}
                </button>
              ))}
            </div>
            ...
            {selectedChars.filter(id => otherChars.find(c => c.id === id)).length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
                {selectedChars
                  .filter(id => otherChars.find(c => c.id === id))
                  .map(id => {
                    const c = otherChars.find(ch => ch.id === id)!
                    return (
                      <button key={id} type="button" onClick={() => toggleChar(id)} style={chipStyle(true)}>
                        {c.canonical_name} ✕
                      </button>
                    )
                  })}
            ...
            <input
              ...
              value={freeTextChar}
              onChange={e => setFreeTextChar(e.target.value)}
              style={{ width: '100%', padding: '9px 12px', borderRadius: 6, border: '1px solid #d4b896', background: '#fff', fontSize: 14, boxSizing: 'border-box' }}
            />
            <p style={{ fontSize: 11, color: '#aaa', margin: '4px 0 0' }}>
              New characteristics are reviewed by the community and promoted once they get enough votes.
            </p>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          <div style={{ background: '#fff', borderRadius: 10, padding: 20, marginBottom: 14, border: '1px solid #e8ddd0' }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '9px 12px', borderRadius: 6, border: '1px solid #d4b896', fontSize: 14, boxSizing: 'border-box' }}
            />
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: 10, padding: 20, marginBottom: 14, border: '1px solid #e8ddd0' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {DURATIONS.map(d => (
                <button key={d.value} type="button" onClick={() => setSmokeDuration(smokeDuration === d.value ? null : d.value)} style={chipStyle(smokeDuration === d.value)}>
                  {d.label}
                </button>
              ))}
            </div>
          ...
          <div style={{ background: '#fff', borderRadius: 10, padding: 20, marginBottom: 14, border: '1px solid #e8ddd0' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <select
                value={pairingCategoryId}
                onChange={e => {
                  setPairingCategoryId(e.target.value)
                  setPairingSubcategoryId('')
                  setPairingSpecific('')
                }}
              ...
              {pairingCategoryId && pairingSubcategories.length > 0 && (
                <select
                  value={pairingSubcategoryId}
                  onChange={e => setPairingSubcategoryId(e.target.value)}
                  style={{ width: '100%', padding: '9px 12px', borderRadius: 6, border: '1px solid #d4b896', background: '#fff', fontSize: 14 }}
                >
                  <option value="">Select a type...</option>
                  {pairingSubcategories.map(s => (
              ...
              {pairingCategoryId && (
                <input
                  type="text"
                  placeholder="Specific (e.g. Yellow Spot 12yr)..."
                  value={pairingSpecific}
                  onChange={e => setPairingSpecific(e.target.value)}
                  style={{ width: '100%', padding: '9px 12px', borderRadius: 6, border: '1px solid #d4b896', fontSize: 14, boxSizing: 'border-box' }}
                />
              )}
            </div>
          ...
          {existingReview && (
            <div style={{ background: '#fff', borderRadius: 10, padding: 20, marginBottom: 14, border: '1px solid #e8ddd0' }}>
              ...
              <textarea
                ...
                placeholder="e.g. Tried a fresh box, updated my scores accordingly..."
                rows={3}
                style={{ width: '100%', padding: '12px 14px', fontSize: 14, border: '1px solid #d4b896', borderRadius: 8, outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'system-ui, sans-serif' }}
              />
            </div>
          )}
        </div>
      ...
      {error && (
        <div style={{ background: '#fbe9e7', color: '#b71c1c', padding: '10px 14px', borderRadius: 6, fontSize: 13, marginTop: 16 }}>
          {error}
        </div>
      )}

      <div style={{ display: 'flex', gap: 12, marginTop: 24, justifyContent: 'flex-end' }}>
        <button type="button" onClick={onCancel} style={{ padding: '11px 24px', borderRadius: 8, border: '1px solid #d4b896', background: '#fff', color: '#5a3a1a', fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>
          Cancel
        </button>
